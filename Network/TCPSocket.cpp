...
TCPSocket::TCPSocket (char * _address, int port ,int readBufferSize,int writeBufferSize)
{
    ...
    sock = socket(AF_INET, SOCK_STREAM, 0); //create an endpoint for communication and return a descriptor 
    ...
    if ( server == NULL ||  server->h_addr == NULL) 
        ...
        bzero((char *) &serv_addr, sizeof(serv_addr)); //allocate all of serv_addr to zero with the size of serv_addr 
        serv_addr.sin_family = AF_INET;
        bcopy((char *)server->h_addr,(char *)&serv_addr.sin_addr.s_addr, server->h_length); //copy byte sequence -> from n amount from src to dest = true when they overlap
